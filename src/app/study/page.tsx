...
import { computeUpcomingReviewOverview } from "@/domain/stats";
...
const EXERCISE_TYPE_LABEL: Record<Exercise["exerciseType"], string> = {
  recall: "中文 → 日文",
  reading: "漢字／日文 → 假名讀音",
  spelling: "拼字",
  translation: "句子翻譯",
...
export default function StudyPage() {
  ...
  function handleGrade(result: AttemptResult) {
    if (isSubmittingRef.current) return; // 防止 grade 按鈕被重複觸發造成重複送出
    const unit = plannedUnits[currentIndex];
    const item = unit ? itemsById.get(unit.learningItemId) : undefined;
    ...
    try {
      ...
      // R4：寫入失敗就停在原題，不前進、不顯示成功；使用者可以直接重按評分按鈕重試
  ...
  if (phase === "summary") {
    ...
    const partialCount = exerciseResults.filter((r) => r.result === "partial").length;
      ...
      <main className="mx-auto flex max-w-md flex-1 flex-col gap-4 px-4 py-6">
        ...
        <section className="rounded-2xl border border-border bg-surface p-4">
          <dl className="grid grid-cols-2 gap-3">
            ...
            <div>
              <dt className="text-xs text-foreground-muted">答對／部分／答錯</dt>
              <dd className="text-xl font-semibold tabular-nums">
                {correctCount} / {partialCount} / {incorrectCount}
              </dd>
            </div>
            <div>
    ...
    <main className="mx-auto flex max-w-md flex-1 flex-col gap-4 px-4 py-6">
      <header>
        ...
        <div className="mt-2 h-1.5 w-full overflow-hidden rounded-full bg-surface-muted">
          <div
            ...
            style={{ width: `${Math.round(((currentIndex + (revealed ? 0.5 : 0)) / total) * 100)}%` }}
          />
        </div>
      ...
      <section className="flex flex-1 flex-col gap-4 rounded-2xl border border-border bg-surface p-4">
        <div className="flex items-center gap-2">
          ...
          <Badge tone="neutral">{EXERCISE_TYPE_LABEL[currentExercise.exerciseType]}</Badge>
        </div>
        ...
        <div className="flex flex-col gap-1">
          ...
          <input
            ...
            placeholder="輸入你的答案，按 Enter 顯示解答"
          />
        </div>
        ...
        {!revealed ? (
          ...
          <div className="mt-auto flex flex-col gap-3">
            <div className="rounded-lg bg-surface-muted px-3 py-3">
              <p className="text-xs text-foreground-muted">正確答案</p>
              <p className="text-xl font-semibold text-foreground">{currentExercise.expectedAnswer}</p>
              {item.explanation ? <p className="mt-1 text-sm text-foreground-muted">{item.explanation}</p> : null}
            </div>
            {gradeError ? (
            ...
            <p className="text-sm font-medium text-foreground">你答對了嗎？</p>
            <div className="grid grid-cols-3 gap-2">
              <button
                type="button"
                disabled={isSubmitting}
                onClick={() => handleGrade("incorrect")}
                className="rounded-xl bg-danger-bg px-3 py-3 text-sm font-semibold text-danger transition-colors hover:opacity-80 disabled:opacity-50"
              >
                答錯
              </button>
              <button
                ...
                className="rounded-xl bg-warning-bg px-3 py-3 text-sm font-semibold text-warning transition-colors hover:opacity-80 disabled:opacity-50"
              >
                部分答對
              </button>
              <button
                ...
                className="rounded-xl bg-success-bg px-3 py-3 text-sm font-semibold text-success transition-colors hover:opacity-80 disabled:opacity-50"
              >
                答對
              </button>
            </div>
